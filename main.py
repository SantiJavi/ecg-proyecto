...
#Parser es un metodo que me permite metidante comandos definir acciones 
...
#Metodo en el que se realiza el entrenamiento. Se realizan 10 epocas
...
#Fase de evaluacion de los datos.
...
if __name__ == "__main__":
    ...
    #establecemos la ruta donde se guardara el modelo entrenado
    if not args.model_path:
        args.model_path = f'models/resnet34_{database}_{args.leads}_{args.seed}.pth'

    if args.use_gpu and torch.cuda.is_available():
        #A torch.devicees un objeto que representa el dispositivo en el que torch.Tensorse asigna o se asignará.
    ...
    #ECG recibe fase,path,label,folds,leads
    #establecemos valores por defecto para los folds:
    #test_folds=7,val_folds=4
    ...
    #Cargo los datos procesados provenientes del entrenamiento
    ...
    #establezco la red neuronal que vamos a utilizr
    net = resnet34(input_channels=nleads).to(device)
    #implementa el algoritmo de Adam para la optimizacion 
    optimizer = torch.optim.Adam(net.parameters(), lr=args.lr)
    #ajuste de la tasa de aprendizaje en funcion del numero de epocas 
    #Decae la tasa de aprendizaje de cada grupo de parámetros por gamma en cada etapa de step_size.
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 10, gamma=0.1)
    #Esta pérdida combina una capa sigmoidea y BCELoss en una sola clase
    #para la estabilidad numerica
